...
class Client:
    """
        ...
        * Cert_key_pem_path: is the path to the private key of the client certificate
        * Cert_key_pem_path: is the path to the private key of the client certificate (None, if the key is embedded with the certificate)
        * Cert_key_password: is the password used to encrypt the client certificate's key (if PEM encrypted).
        * Note: if key_pem_path is null, the signer will try to use the private key from the certificate.
    """
    def __init__(self, scheme, host, port, key_pem_path, key_password = None, cert_ca_path = None, cert_pem_path=None, cert_key_pem_path=None, cert_key_password = None):
    ...
    def commit(self, Method, Path, Body=None):
        print(f'{Method.upper()} {Path}: {Body}')
        try:
            if self.scheme == 'http':
    ...
    def create_user(self, Username, Email, Key=None):
        if Key is None:
            Key = self.signer.public_key_b64.decode()
        body = self.signer.signObject(models.User(Username, Email, Key), withDigest = self.withDigest)
    ...
    def group_add_user(self, GroupId, UserId, Token=None):
        if Token is not None:
            return self.commit('POST', f'/v1/group/{GroupId}/user/{UserId}?secret={Secret}')
        else:
    ...
    def create_module(self, Name):
        body = self.signer.signObject(models.Module(Name), withDigest = self.withDigest)
        return self.commit('POST', f'/v1/module/', body)

    def get_module(self, Id):
        return self.commit('GET', f'/v1/module/{Id}')

    def update_module(self, Id, Name=None):
        body = self.signer.signObject(models.Module(Name), params=[Id], withDigest = self.withDigest)
        return self.commit('POST', f'/v1/module/{Id}', body)
    ...
    def create_question(self, ModuleId, Name, Before, Grader, MaxScore, MinScore):
        body = self.signer.signObject(models.Question(Name, Before, Grader, MaxScore, MinScore), params=[ModuleId], withDigest = self.withDigest)
        return self.commit('POST', f'/v1/question/module/{ModuleId}', body)

    def get_question(self, Id):
        return self.commit('GET', f'/v1/question/{Id}')

    def update_question(self, Id, Name=None, Before=None, Grader=None, MaxScore=None, MinScore=None):
        body = self.signer.signObject(models.Question(Name, Before, Grader, MaxScore, MinScore), params=[Id], withDigest = self.withDigest)
        return self.commit('POST', f'/v1/question/{Id}', body)
